...
- When developing, run focused tests in ./test folder with `julia --project=. -t 2 test/runtests.jl <test file name>`.
- Run the full package test suite with `julia --project=. -t 2 test/runtests.jl`.
...
- If you change docs content or public APIs, also verify the docs build in ./docs folder with `julia --project=. docs/make.jl`.

## Code Style
- Use 4 spaces for indentation.
- Keep implementations small and type-stable where practical.
- Prefer explicit, readable linear algebra.
- Update docstrings and docs pages when changing public APIs in `src/`. Use `$(SIGNATURES)` and `$(FIELDS)` from DocStringExtensions in docstrings.
- When adding or removing functions/types with docstrings, update `docs/src/api.md` so every docstring is included in a `@docs` block — the docs build fails on missing entries.
- When changing user-facing API (e.g. function signatures), update the README Quick Start example and any `examples/` files to match.
...
- Ensure CI-equivalent checks pass locally:
  - Full package tests
  - Docs build (`cd docs && julia --project=. make.jl`) — always verify, not just when you think docs changed
...
# Build documentation (must run from docs/ directory)
